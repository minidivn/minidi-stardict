...
function getArtInfoGzip(pos, len, ab, gzip_header, vccc) {
    const ArtInfo = [];
    ...
    for (
        var i = firstchunk, j = 0; i <= lastchunk && j < in_buffer.byteLength; j += chunks[i][1], i++
    ) {
...
function getArticleBodyFromDZOL(dzfile, pos, len) {
    let tab = getSliceChunksTable(dzfile, [
        ['', parseInt(pos), parseInt(len)]
    ]);
    return getArticleBodyFromDZByInfo(dzfile, JSON.parse(tab[0][1]));
}

function getArticleBodyFromDZByInfo(dzfile, ArtInfo) {

    let bufSize = ArtInfo[0][1] - ArtInfo[0][0]
    if (ArtInfo.length === 2) {
        let last = ArtInfo[1];
        ArtInfo[1] = [
            [0, bufSize]
    ...
    fs.readSync(fd, buf, 0, bufSize, ArtInfo[0][0]);
    fs.closeSync(fd);
    ...
    */
    ...
    for (let i = 0; i < ArtInfo[1].length; i++) {
        ...
        let chunkBuf = new Buffer(chunk);
        // console.log(chunk);
        let inflated = zlib.inflateRawSync(chunkBuf, {
...
function readIndexFile(path, offsetLength, ignoreCase) {

    let indfile
    let synfile

    // NOTE: Detect file by path
    if (fileExists(path)) {
        indfile = path;
    } else {
    ...
    while (i < buf.length) {
        ...
        offsetLength.push([offset, size, word]);
...
function icase(word, ignoreCase) {
    if (ignoreCase === true) word = word.toLowerCase();
    return word.trim();
}

function searchArtInfoFromIndexFile(indexfile, searchTerm, ignoreCase, cachedWordList) {
    var offsetLength = [];
    let resultOL = [];

    if (cachedWordList) {} else {
        cachedWordList = [];
        readIndexFile(indexfile, cachedWordList, ignoreCase);
    }
    offsetLength = cachedWordList;

    // FIXME: Find one by one! Slow!
    for (var i = 0; i < offsetLength.length; i++) {
        var wordData = offsetLength[i];
        var offset = wordData[0];
        var size = wordData[1];
        var word = wordData[2];
        if (icase(searchTerm, ignoreCase) === icase(word, ignoreCase)) {
            resultOL.push([offset, size]);
            break;
        }
    }
...
function getArticleBodyFromDZByQueryFile(dzfile, indexfile, searchTerm, ignoreCase, cachedWordList) {
    indexfile = indexfile || dzfile.replace(/(\.dict\.dz)$/i, '');
    let resultOL = searchArtInfoFromIndexFile(indexfile, searchTerm, ignoreCase, cachedWordList);
    if (resultOL.length === 1) {
        return getArticleBodyFromDZOL(dzfile, resultOL[0][0], resultOL[0][1]);
    } else {
        return false;
    }
}

function getArticleBodyFromTextDictByQueryFile(dzfile, indexfile, searchTerm, ignoreCase, cachedWordList) {
    indexfile = indexfile || dzfile.replace(/(\.dict\.dz)$/i, '');
    let resultOL = searchArtInfoFromIndexFile(indexfile, searchTerm, ignoreCase, cachedWordList);
    if (resultOL.length === 1) {
        return getArticleBodyFromTextDictOL(dzfile, resultOL[0][0], resultOL[0][1]);
    } else {
        return false;
    }
}

function getArticleBodyFromTextDictOL(dictfile, offset, length, encoding = 'utf8') {
...
module.exports = {
    ...
    getArticleBodyFromDZOL: getArticleBodyFromDZOL,
    getArticleBodyFromDZByInfo: getArticleBodyFromDZByInfo,
    getArticleBodyFromDZByQueryFile: getArticleBodyFromDZByQueryFile,
    getArticleBodyFromTextDictByQueryFile: getArticleBodyFromTextDictByQueryFile,
    getArticleBodyFromTextDictOL: getArticleBodyFromTextDictOL
}